...
import com.example.capstone_ryecyclerview_ex.search_view.restaurant_MainActivity;
import com.example.capstone_ryecyclerview_ex.search_view.tour_MainActivity;
import com.example.capstone_ryecyclerview_ex.u_map1;
import com.example.capstone_ryecyclerview_ex.u_map2;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        */
        ...
        next_button.setOnClickListener(v -> {
            ...
            if(cnt==0){
                Intent intent2=new Intent(MainActivity.this, u_map1.class);
                startActivity(intent2);
                cnt+=1;
            }
            else{
                Intent intent2=new Intent(MainActivity.this, u_map2.class);
                startActivity(intent2);
            }


            //여기에 서버 연결 함수 쓸 것
            //connection.setRequestProperty("function","insert");
            //connection.setRequestProperty("result_time", result_time);
            //connection.setRequestProperty("result_list", result_list);

        });
        //list(str):장바구니 순서데로 정보 담음
        //list ="순서 타입 이름 주소 체류시간 (여행지번호) ... 순서 타입...."
        //date+start_time(str)+last_time(str) "여행날짜+시작시간+종료시간"정보 따로 보내주기
        //date처리 아직 못함
        ...
        /*get intent 위치별로 하고 싶은디...
        //intent 데이터 받아오기
        Intent get_intent = getIntent();
        String[] str = (String[]) get_intent.getSerializableExtra("arrayd"); //이중 배열 받기 "0","박물관","주소" 이렇게 온다고 가정
        setadapter(str[0],str[1],str[2]); //list.add 추가 함수

        setadapter("2","짱구는 ","못 말려");
        */
...
//
    ...
    public void setadapter(String type, String name , String adress){
        count++;
        //Toast.makeText(v.getContext(), "맛집", Toast.LENGTH_LONG).show();
        list.add(new ItemModel(Integer.parseInt(type), count+name,adress,11,"1",x,y));
        return;
    }
    //
