...
public class restaurant_ItemAdapter extends RecyclerView.Adapter<restaurant_ItemAdapter.ItemViewHolder> implements Filterable {

    private List<restaurant_ItemModel> mDataList;
    private List<restaurant_ItemModel> mDataListAll;

    //constructor
    public restaurant_ItemAdapter(List<restaurant_ItemModel> items) {
        mDataList = items;
    ...
    //data set changed
    public void dataSetChanged(List<restaurant_ItemModel> exampleList) {
    ...
    public ItemViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.tour_row_item,
                parent, false);
    ...
    public void onBindViewHolder(@NonNull final ItemViewHolder holder, final int position) {
        restaurant_ItemModel currentItem = mDataList.get(position);
    ...
    private Filter exampleFilter = new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            List<restaurant_ItemModel> filteredList = new ArrayList<>();

            if (constraint == null || constraint.length() == 0) {
                filteredList.addAll(mDataListAll);
            } else {
                String filterPattern = constraint.toString().toLowerCase().trim();
                for (restaurant_ItemModel item : mDataListAll) {
