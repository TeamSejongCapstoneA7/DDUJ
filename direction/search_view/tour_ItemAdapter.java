...
public class tour_ItemAdapter extends RecyclerView.Adapter<tour_ItemAdapter.ItemViewHolder> implements Filterable {

    private List<tour_ItemModel> mDataList;
    private List<tour_ItemModel> mDataListAll;

    //constructor
    public tour_ItemAdapter(List<tour_ItemModel> items) {
        mDataList = items;
    ...
    //data set changed
    public void dataSetChanged(List<tour_ItemModel> exampleList) {
    ...
    public void onBindViewHolder(@NonNull final ItemViewHolder holder, final int position) {
        tour_ItemModel currentItem = mDataList.get(position);
    ...
    private Filter exampleFilter = new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            List<tour_ItemModel> filteredList = new ArrayList<>();

            if (constraint == null || constraint.length() == 0) {
                filteredList.addAll(mDataListAll);
            } else {
                String filterPattern = constraint.toString().toLowerCase().trim();
                for (tour_ItemModel item : mDataListAll) {
